...
> 本文档是对 PhantomFilmer `be73305` 基线的从功能出发、由外到内的代码调研；以源码为唯一事实源。
...
**PhantomFilmer** 是一个面向 **RoboMaster TT / Tello Talent** 无人机的自动跟拍系统 **Python 原型**。核心能力：开启视频流 → 人物 ReID → 起飞并闭环到基础悬停高度 → 普通/侧向/前向三种自动跟随 → 目标丢失时有界搜索 → 可选前向 ToF 避障 → 电量/高度/超时安全降落。仓库还提供由旧 WebUI 基础迁移而来的 Electron GUI，但当前只用于预检、起降、视频和手动 RC，尚未覆盖 CLI 主能力。
...
桌面端不属于 `--mode` 矩阵，也不运行 ReID 自动跟随；它是独立的、覆盖范围有限的真机手动控制入口。架构和缺口见 [07-桌面端架构与安全边界](07-桌面端架构与安全边界.md)，安装、预检和操作流程见 [桌面端真机操作指导书](桌面端真机操作指导书.md)。
