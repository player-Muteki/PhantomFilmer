...
> **重要边界**：`KernelSession._emit` 是“正常自治运动”的单一出口，但**不是全仓库唯一 `move_rc` 调用点**。兼容/安全路径（`FollowSession._safe_zero_output` [control/follow_session.py:1282](../control/follow_session.py#L1282)、Console 的 `_send_safe_rc`）也会直接发送经限幅的零命令。
