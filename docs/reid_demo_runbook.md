...
7. 进入跟随后如果从未识别到目标，无人机直接从当前高度开始固定三层扫描；跟随中途丢失时才使用目标最后水平方向。每层依据飞控 yaw 累计完整旋转 360°，相邻层反向旋转；找到候选人后连续 5 帧确认身份才恢复跟随。
...
起飞后，视频流、TOF、高度范围或检测器连续异常仍会触发清零和安全降落；是否已经看到目标不再作为起飞门禁。
