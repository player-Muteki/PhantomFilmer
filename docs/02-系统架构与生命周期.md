...
flowchart TD
    ...
    WEBSTART[scripts/start_webui.sh] --> WEBUI[webui Next.js]
    WEBSTART --> WEBAPI[web_api/server.py]
    WEBUI -->|/api/drone/* 反向代理| WEBAPI
...
- **两个运行入口**：CLI 自动跟随通过 `FollowSession/KernelSession`；本地 WebUI 通过 `DroneWebService` 直接拥有一个真机适配器，只提供手动 RC，不进入 ReID 与内核 phase FSM。
...
`scripts/start_webui.sh` 启动两个本机进程：

```text
浏览器 :3000 → Next.js /api/drone/* 代理 → ThreadingHTTPServer :8765
                                         ↓
                                  DroneWebService
                                         ↓
                                  RealTelloAdapter
...
0.4 s RC 失联看门狗和 0.2 s 前向 ToF 轮询。HTTP 只绑定 `127.0.0.1`；服务在收到
`POST /api/drone/connect` 前不会连接无人机或请求视频。
...
- **WebUI**：`DroneWebService.move_rc/_send_hover` 使用本地服务常量限幅和看门狗，是另一个独立运行时出口。
...
- **WebUI 与 CLI 不共享会话状态**：两个入口复用适配器代码，但不能同时控制同一架无人机。
