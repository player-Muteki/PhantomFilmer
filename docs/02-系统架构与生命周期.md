...
- **统一运行时边界**：CLI Console 与桌面 sidecar 都通过 `MissionManager.execute()` 接收强类型命令，并发布带单调序号的事件与 `RuntimeSnapshot`。桌面端的普通跟随、ReID 与 CLI Console 任务由同一个 `MissionFactory` 构造 `FollowSession`；差异集中在入口、资源所有权和 GUI 地面确认门。详见 [07](07-桌面端架构与安全边界.md)。
...
`app/runtime/` 是 CLI 与 GUI 的共同应用边界：

- `commands.py`：设备、飞行、任务和控制模式的强类型命令。
- `mission_manager.py`：串行执行命令，成功或拒绝都发布权威快照。
- `event_bus.py`：线程安全、单调序号、最多保留 1024 条事件并支持订阅/补读。
- `models.py`：英文稳定码的 phase、mission、control mode、allowed actions 与遥测快照。
- `mission_factory.py`：统一构造普通、ReID 与 Console 的 `FollowSession`。

`ConsoleTools` 和 `DroneWebService` 都继承 `MissionManager`。版本化 HTTP 契约已覆盖设备、
飞行、任务、地面预览和模式控制命令；事件按序号增量补读，历史缺口由客户端重取快照。

### 2.2 桌面端独立设备会话

Electron 主进程启动并监管一个本机 sidecar；自动任务在 sidecar 中进入共享自治控制链路：

```text
renderer → 类型化 preload IPC → Electron main → 带会话令牌的随机回环端口
                                              ↓
                         ThreadingHTTPServer → MissionManager / DroneWebService
                                              ↓
                                  RealTelloAdapter / TelloDroneAdapter
```

`DroneWebService` 用 `RLock` 保护适配器与遥测状态，并启动三个 daemon 线程：
0.4 s RC 失联看门狗、0.2 s 前向 ToF 轮询和独立手动飞行遥测监视。HTTP 只绑定 `127.0.0.1` 且所有接口要求
会话认证；服务在收到 `/api/v1/commands` 的 `device.connect` 前不会连接无人机或请求视频。
v1 提供权威快照、按序号补读事件和 1 秒 RC 独占租约；旧 `/api/...` 端点暂时仅作兼容。

它加载 `config.yaml` 的共享安全与任务配置。独立手动飞行在服务层执行租约、方向门和持续
遥测降落保护；自动任务构造 `SafetyManager`、视觉检测器、`FollowSession`、
`KernelSession` 与 `ArbitrationEngine`。地面预览在已有 MJPEG 流上执行 ReID 调试叠加，
确认后把同一检测器移交给任务，预览、手动起飞和自动任务严格互斥。
...
- 构造签名与公共 API（`send_command`、`handle_key`、`request_stop`、`request_emergency_stop`，均定义于 [control/follow_session.py](../control/follow_session.py)）。
...
stateDiagram-v2
    ...
    CONTROL_READY --> FOLLOW: m（手动）/ a（普通）/ s（侧向）/ f（前向）
...
| `ControlReadyHandler` | `phase_handlers/control_ready.py` | 基础高度持续悬停；`m/a/s/f` 均进入 FOLLOW |
...
- **桌面端不是 CLI 模式逐项翻译**：它覆盖真机 ReID、自动跟随、搜索、可选避障与生命周期，但不暴露 Fake、dry-run、算法自测或 Console。
