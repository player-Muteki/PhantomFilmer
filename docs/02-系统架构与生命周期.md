...
- **统一运行时边界**：CLI Console 与桌面 sidecar 都通过 `MissionManager.execute()` 接收强类型命令，并发布带单调序号的事件与 `RuntimeSnapshot`。桌面端的普通跟随、ReID、固定航线与 CLI Console 任务由同一个 `MissionFactory` 构造 `FollowSession`；差异集中在入口、资源所有权和 GUI 地面确认门。详见 [07](07-桌面端架构与安全边界.md)。
...
- `mission_factory.py`：统一构造普通、ReID、固定航线与 Console 的 `FollowSession`。
...
stateDiagram-v2
    ...
    CONTROL_READY --> FOLLOW: m（手动，跳过固定航线）/ s（侧向）/ f（前向）
...
| `ControlReadyHandler` | `phase_handlers/control_ready.py` | 基础高度持续悬停；`m/s/f` 进 FOLLOW，`a` 进 PRE_FOLLOW |
