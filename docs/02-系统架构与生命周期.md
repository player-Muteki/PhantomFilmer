...
- **两个不对等的运行入口**：CLI 自动跟随通过 `FollowSession/KernelSession`，承载仓库主能力；由旧 WebUI 基础迁移而来的桌面 sidecar 通过 `DroneWebService` 直接拥有一个真机适配器，只提供手动 RC 子集，不进入 ReID 与内核 phase FSM。详见 [07](07-桌面端架构与安全边界.md)。
