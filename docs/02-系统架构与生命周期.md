...
- **统一运行时边界，能力仍不对等**：CLI Console 与桌面 sidecar 都通过 `MissionManager.execute()` 接收强类型命令，并发布带单调序号的 `command.accepted/completed/rejected` 事件与 `RuntimeSnapshot`。普通跟随、ReID、固定航线和 Console 任务由同一个 `MissionFactory` 构造 `FollowSession`。但桌面 sidecar 当前仍只提供手动 RC 子集，尚未调用 ReID 与内核 phase FSM。详见 [07](07-桌面端架构与安全边界.md)。
...
`ConsoleTools` 和 `DroneWebService` 都继承 `MissionManager`。这一步统一了操作入口和状态
语义，但尚未让桌面端启动自动任务；版本化 HTTP/SSE 契约在后续阶段接入。

### 2.2 桌面端独立设备会话

Electron 主进程启动并监管一个本机 sidecar。该链路是 WebUI 的桌面化移植，不覆盖 CLI
自治控制链路：
...
它不加载 `config.yaml`，不构造 `SafetyManager`、视觉检测器、`FollowSession`、
`KernelSession` 或 `ArbitrationEngine`；桌面端安全阈值和状态机均在 `web_api.server` 中
单独实现。
...
- **桌面端不是 CLI GUI**：当前只是旧 WebUI 的基础 Electron 迁移，未覆盖 CLI 的 ReID、自动跟随、目标搜索、自动避障、Fake 和完整生命周期。
