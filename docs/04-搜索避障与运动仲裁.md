...
>
> 本文以普通跟随的 `ArbitrationEngine` 配方表为主事实源，说明三层有界搜索、前向 ToF 缓存、`MotionArbiter`、`ObstacleAvoidancePlanner` 的逐 tick 优先级与落地语义，以及朝向模式旁路、中心丢失前探、异步 JSONL 日志和传感器盲区。
...
| 3 | 手动接管有效 | `ManualFeature.propose`；只应用前向 ToF/高度门，不调用 `MotionArbiter` | 无 |
...
| 4 | 目标丢失 + 障碍存在 | `ObstacleFeature.own`（OBSTACLE_FIRST） | [arbitration.py:109](../control/kernel/arbitration.py#L109) |
| 5 | 目标丢失 + 无障碍 + 搜索开启 | `SearchFeature.propose` | [arbitration.py:165](../control/kernel/arbitration.py#L165) |
| 6 | 目标丢失 + 无障碍 + 搜索关闭 | `SafetyFeature.lost_hover` | 同上 |
| 7 | 目标存在 | `FollowFeature.propose` → `ObstacleFeature.arbitrate`（`obstacle_priority=False`） | [arbitration.py:145](../control/kernel/arbitration.py#L145) |

要点：

- 每 tick **只选一个所有者**，未选中的 feature 冻结内部状态机与计时器（[arbitration.py:1](../control/kernel/arbitration.py#L1)）。
- 手动所有权高于首次目标门、搜索和自动避障；前方过近只把手动前进压成零，不启动右移路线。
...
当 `follow_mode in {"side", "front"}`、未手动接管且不在模式切换时，
`FollowSession._loop` 不调用 `ArbitrationEngine`，而是直接调用共用的朝向跟随路径：

```text
ReID + JointBDOE → SideFollowController → RC
          └─持续丢失 0.5 s─→ TargetSearchController → 找回确认或落地
```

因此侧向/前向模式不运行 `MotionArbiter`、顶部 ToF 自动避障、中心丢失前探或
`ObstacleAvoidancePlanner`。电量、高度、视频失败、停止、急停和最终限幅仍由会话层执行。
自动飞行按 `1/2/3` 切换模式时先悬停；连续确认目标前，模式切换路径也会复用搜索。
...
- **朝向模式复用**：侧向/前向不再维护独立的横移 3 s、旋转 360°、悬停 3 s 恢复状态机；丢失确认后进入本节同一个三层搜索。找回后清空旧朝向选择，重新稳定采样。
...
朝向跟随另由 `SideFollowEventRecorder` 写 `logs/side_follow/*.jsonl`，侧向和前向模式
复用同一记录器结构。它逐 tick 记录目标角度、模式、位置优先、绕行横移/偏航分量、
搜索状态、遥测和最终 RC；同样使用有界队列，不能视为无损审计。真正的起飞/降落
SDK 调用审计位于 `logs/flight_events/*.jsonl`，由真机适配器同步写入。
...
- **朝向模式无自动避障**：这是显式旁路，不是 ToF 故障；真机只能依赖场地净空和人工接管。
