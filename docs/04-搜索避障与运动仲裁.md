...
ReID 场景结果可提供 `障碍物候选：类别`。`VisualObstacleAdvisor` 只消费已有 YOLO 结果，不重复推理；中央候选触发 `VISUAL_CAUTION` 并限制正向速度，连续面积增长达到配置帧数后标记为接近。视觉只能升级风险，不能解除红外 `IR_CAUTION`、`IR_BLOCKED` 或 `IR_UNKNOWN`。
...
- `distance_cm <= front_tof_blocked_distance_cm`（当前 60 cm）→ `IR_BLOCKED`，置 `found=True, state=BLOCKED` 并进入既有确认/绕行流程。
- `blocked < distance_cm <= front_tof_caution_distance_cm`（当前 100 cm）→ `IR_CAUTION`，停止正向运动但保留后退、上下和偏航。
- `out_of_range` 表示量程内没有实体，保持 CLEAR；`stale/error/not_ready` 进入 `IR_UNKNOWN`，悬停且不得由视觉解除。
