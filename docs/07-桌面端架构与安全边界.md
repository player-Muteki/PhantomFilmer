...
- **渲染层结构**：`App.tsx` 收敛为纯编排，界面拆为 `components/` 下八个组件；左栏改为
  ...
  起飞按钮常驻 tab 外。顶部 `FlightCommandBar` 将后端/真机连接、飞行状态、模式选择、
  暂停、遥测陈旧与安全告警收敛为固定命令栏；手动控制板改为视频内半透明 HUD 叠加，事件
  时间线移入 tab。
...
- **新增操作面**：人物跟拍任务由界面固定传入 `mission.start`；
  暂停/继续（按钮 + `P` 键）；两步建档（选照片→确认）以及断开状态下的更新照片、重命名
  和删除；带类别筛选与毫秒时间戳的运行事件时间线；命令栏断开真机入口；手动 HUD 内键盘
  控制开关；输入框聚焦时飞行键失效。
