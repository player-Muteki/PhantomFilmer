...
Electron main 启动 `SidecarManager`，sidecar 在收到显式 `POST /api/drone/connect` 前不会连接无人机或请求视频。主要调用链如下：

| 操作 | HTTP 路径 | sidecar 行为 |
|------|-----------|--------------|
| 健康检查 | `GET /api/health` | 验证进程与认证链路 |
| 连接 | `POST /api/drone/connect` | SDK 握手、视频启动、初始遥测与前向 ToF |
| 状态 | `GET /api/drone/status` | 刷新缓存电量、底部高度和预检状态 |
| 起飞 | `POST /api/drone/takeoff` | 五项预检通过后预授权适配器并起飞 |
| RC | `POST /api/drone/rc` | 校验、限幅、方向安全门后发送一次 RC |
| 悬停 / 降落 | `POST /api/drone/hover`、`land` | 清零；正常降落保留连接与视频 |
| 停止 / 紧急降落 | `POST /api/drone/stop`、`emergency-land` | 空中先降落，再关闭会话 |
| 安全退出 | `POST /api/sidecar/shutdown` | 停止看门狗、降落/断开并退出进程 |

HTTP handler 会把连接、状态、起降、悬停、RC、停止和急停请求转换为 `app/runtime/commands.py` 的强类型命令，再通过 `MissionManager.execute()` 串行执行。每条命令发布单调序号事件和 `RuntimeSnapshot`；旧 `/api/...` 响应格式目前保持兼容，事件尚未开放给 renderer。
