...
人物档案保存在 Electron `userData/reid_profiles`。renderer 只能请求系统文件选择器，所选路径由 Electron main 发送给认证 sidecar；建档只允许在无人机断开时执行。capability 始终列出已实现任务，并通过 `missionReadiness.available/missingAssets` 单独报告 YOLO、OSNet 和 JointBDOE 权重是否就绪；缺少资产时 GUI 禁止启动任务。
