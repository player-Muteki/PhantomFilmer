...
flowchart LR
    ...
    HTTP --> SERVICE[Python DroneWebService]
...
`SidecarManager` 保存最近一次成功 HTTP 响应中的 `airborne`。sidecar 意外退出时，若该值为真，会锁定所有控制并禁止自动重启；现有 E2E 覆盖了“已收到空中状态后崩溃”的路径（[desktop/e2e/app.spec.ts:39](../desktop/e2e/app.spec.ts#L39)）。
