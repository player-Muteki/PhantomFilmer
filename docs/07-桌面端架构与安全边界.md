...
renderer 现按用途拆分为三个工作区：

- **飞行控制**：保留连接、预检、视频、起降和手动 RC 主流程。
- **任务与人物**：展示人物与任务工作流，但按钮只按 sidecar 的 `missions` capability 启用；当前仅 `manual` 可操作，自动跟随、ReID 跟拍和固定航线仍保持禁用。
- **运行诊断**：展示 Python 权威快照、允许操作、RC 租约参数及最近 50 条运行时事件。
...
自动任务的底层会话现支持 `OperatorCommandChannel`：GUI 使用语义命令选择手动、普通、侧向或前向模式，并可发送暂停、悬停、短时手动方向、停止和急停；`FollowSession` 在无 OpenCV 窗口时仍持续执行电量、高度、视频和安全检查。邮箱有界，停止/急停会清除排队中的旧运动指令；急停在下一次帧读取或推理前处理。该通道已经进入 `MissionFactory` 装配路径，但本阶段尚未把自动任务 capability 对外开放。

sidecar 后续启动自动任务时可让 `CameraStream` 附着于已有视频流：任务结束只结束自身读取状态，不关闭由桌面服务持有的 MJPEG 流。CLI 保持原有的独占启停行为。
...
- 工作区、手动控制与二次确认：[desktop/src/renderer/src/App.tsx](../desktop/src/renderer/src/App.tsx)
