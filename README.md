...
基于 RoboMaster TT / Tello Talent 的无人机自动跟拍系统原型。完整能力仍以 Python CLI
为主；当前 Electron GUI 只是原 WebUI 的基础桌面化移植，只覆盖真机手动控制子集。
...
- 到达 150 cm 后会一直悬停等待选择：`m` 手动、`a` 普通自动、`s` 侧向；没有默认选择或超时。当前前向模式请先按 `a`，进入自动后再按 `3`
...
4. 到达 150 cm 后持续悬停，可在窗口按 `m` 手动、`a` 原自动跟随或 `s` 侧向跟随；等待期间不运行 ReID、搜索或避障。侧向模式自动选择最近的 `90°/270°`。界面虽显示 `f`，但当前生命周期处理器尚未接住该初始返回值，因此前向模式应先按 `a`，再按 `3` 进入。
...
仓库包含由旧 WebUI 迁移而来的 Electron 桌面飞控台。它目前只覆盖真机连接、内嵌视频、
五项起飞预检、二次确认起降、悬停、手动 RC、失联看门狗、后端诊断和安全退出，**没有
