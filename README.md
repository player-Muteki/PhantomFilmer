...
- 到达 150 cm 后会一直悬停等待选择：`m` 进入手动，`a` 进入自动；没有默认选择或超时
...
4. 到达 150 cm 后持续悬停，可在窗口按 `m` 手动、`a` 原自动跟随、`s` 侧向跟随或 `f` 前向跟随；等待期间不运行 ReID、搜索或避障。`s/f` 仅在 JointBDOE 及对应模式均开启时显示。侧向模式自动选择最近的 `90°/270°`，前向模式固定以圆周最短方向接近 `180°`。
...
它按设计不运行障碍检测或绕障。人物横向出画后先沿最后出画方向横移 3 秒；仍未找到
就沿同方向根据飞控 yaw 累计旋转 360°，无 yaw 时用 12 秒计时兜底；再找不到则悬停
3 秒并降落。任何阶段重新识别到人物都会立即停止盲搜，恢复横移直到人物居中。
...
侧向跟随每次按 `s` 启动后，会异步写入 `logs/side_follow/*.jsonl`。日志逐控制帧记录人物中心与朝向、90°/270°侧面选择、转身稳定判定、位置优先、丢失搜索阶段、横移方向、旋转进度、横移/偏航分量、电量、高度、机体 yaw 和最终 RC 指令；队列满或写盘失败时只丢弃日志，不影响飞控。

侧向目标丢失后直接复用普通 ReID 跟随的 `target_search`：先短暂悬停，按需执行
有限后退和最后方向偏航，再依次在当前、上方 20 cm、下方 20 cm 三层各旋转
360°，最后返回起始高度并降落。重新识别必须连续确认 5 帧；确认后清空旧侧面，
重新采集 JointBDOE 角度并恢复侧向跟随。侧向搜索仅保留一个例外：不进入避障仲裁。
