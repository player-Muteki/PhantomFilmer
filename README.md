...
人物框默认显示目标角色标签。若要在 ReID 画面中额外检测车辆、椅子、背包等视觉障碍候选，先完成 CPU 性能测试，再将 `vision.visual_object_detection_enabled` 改为 `true`。启用避障后，这些候选只能触发视觉预警和正向限速；红外 ToF 仍是阻挡、绕行与传感器故障保护的最高优先级，视觉不能解除红外风险。
...
所有避障状态都禁止前进。普通 BLOCKED 和目标丢失遮挡路线都固定以速度 20 向右平移约 1 m（当前按约 5 秒航位推算），然后以速度 12 左转最多 90 度寻找目标。转满后交回跟随/搜索仲裁；目标丢失恢复路线中，ReID 重新确认目标会恢复正常跟随。Tello 没有横向里程计，因此 1 m 是速度—时间估算，会受电量、风和地效影响。顶部 ToF 不能保护后方、侧方、上方或下方；真机启用前必须人工确认这些方向净空，并随时准备按 `e` 急停。
