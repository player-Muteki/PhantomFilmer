...
4. 进入跟随后若尚未稳定锁定目标，以 `yaw=+20` 沿固定方向原地旋转一整周，禁止前后、横移和升降；飞控 yaw 累计到 360°前，只要出现候选目标就立即悬停验证。连续可信识别 10 帧后才允许跟随；一周结束仍未锁定或总耗时达到 35 秒则降落。
5. 已锁定目标突然消失时，先用最后目标框和运动趋势预测其位置。全局 `BLOCKED` 障碍，或面积至少占画面 2% 且与预测目标框连续重合的 `CAUTION` 障碍，连续关联 3 帧且一侧自由空间足够时，才执行一次 `±25` 横移脉冲。横移约 4.0 秒（名义约 1 m）、原航向悬停 0.55 秒重新识别；若仍未找到，则朝横移反方向以 `yaw=±20` 偏航 30°、短暂停留运行 ReID，再闭环回到横移前航向。候选目标连续通过 5 帧身份确认后才恢复跟随；若遮挡已确认但左右都不安全，则悬停 3 秒等待通道恢复，随后降落，禁止误入全局 360° 搜索。
6. 目标丢失时先悬停 1 秒；疑似过近时以 `-35` 后退 1.5 秒、停顿 0.5 秒，最多两次。之后只按最后水平方向以 `±25` 偏航，再按“当前高度 → 上方 20 cm → 下方 20 cm”的固定顺序分层搜索。每层以 `±20` 偏航，并根据飞控 yaw 累计完整旋转 360°；相邻层反向旋转。三层完整搜索一轮后返回起始高度，仍未找回则清零并降落，不再使用总时间限制。
...
该入口强制开启避障、逐帧检测和决策日志，且要求输入精确授权口令 `RECOVERY-TEST` 后才会继续；它不接受 `--fake`。
...
目标丢失后不再因为“画面里有任意障碍”就主动绕行。系统先检查任务是否曾稳定锁定目标，再将最后目标框按短时速度预测，并用目标框重叠率与障碍框 IoU 做连续帧关联。全局 `BLOCKED`，或面积至少 2% 且与预测目标位置连续关联的 `CAUTION`，才可能触发遮挡恢复；接管命令只含左右横移，不叠加前进、升降或偏航。未关联的小轮廓不会触发横移。详细验收步骤见 [`docs/reid_recovery_real_test.md`](docs/reid_recovery_real_test.md)。
