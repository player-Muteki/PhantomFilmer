...
4. 锁定后继续复用同一个预览窗口；目标仍有效时按 `Y` 确认起飞，按 `Q` 取消。窗口在确认和起飞稳定阶段持续刷新，不再阻塞等待终端输入。
5. 起飞后继续复用同一视频流和窗口，并进入普通跟随、安全限速和 ReID 目标丢失搜索。
6. 目标丢失时先悬停 1 秒；疑似过近时最多低速短后退两次，否则按最后偏差方向偏航或升降，再在 80～200 cm 范围内分层偏航搜索。重新识别必须连续通过 5 帧；总计 30 秒仍未找回则清零并降落。
...
ReID 丢失搜索为了保持动作可预测，不让普通前视避障规划器改写搜索轨迹。尤其是“过近恢复”的短后退无法由前置摄像头确认机后净空；真机启用前必须确认后方、上方和下方搜索空间无遮挡，并随时准备按 `e` 急停。
