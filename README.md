...
5. 起飞后继续复用同一视频流和窗口，并进入普通跟随、目标丢失降落、安全限速和已选择的避障管线。
...
- 自动跟随会话起飞后先闭环到达 `base_hover_height_cm`（当前为 150 cm），连续稳定后才进入跟随；ReID 演示在起飞稳定和升高期间保持识别，身份丢失时停止升高并悬停，重新连续确认后才恢复；测高、视频、识别失败或超时则降落
