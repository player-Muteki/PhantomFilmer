...
侧向跟随会先悬停采集 5 帧稳定朝向，之后锁定更近的一侧，不在飞行中切换
`90°/270°`。角度稳定时，人物在画面内左右移动会让飞机同方向横移，机头不转；
人物转身导致朝向角连续偏离后，飞机才横移绕人并用偏航保持人物居中，恢复锁定
侧面后重新进入平行横移。它按设计不运行障碍检测或绕障；目标丢失会沿用悬停后
降落保护。首次真机测试必须低速、宽阔、带保护罩。跑道平移方向错误时调整
`tracking_lateral_direction_sign`，绕人角度越走越远时调整 `lateral_direction_sign`。
