...
5. 目标丢失时先悬停 1 秒；疑似过近时以 `-35` 后退 1.5 秒、停顿 0.5 秒，最多两次。之后只按最后水平方向以 `±25` 偏航，再按“当前高度 → 上方 20 cm → 下方 20 cm”的固定顺序分层搜索。每层以 `±20` 偏航，并根据飞控 yaw 累计完整旋转 360°；相邻层反向旋转。总计 75 秒仍未找回则清零并降落。
...
目标丢失且检测到阻挡障碍时，避障优先：暂停 ReID 丢失搜索，主动绕行（前进恒为 0，仍先经时间确认再绕行，5 秒无法绕开则 fail-safe 降落）；障碍清除后再恢复丢失处理、重新触发找人。无障碍时 ReID 搜索轨迹仍不被普通前视避障规划器改写，以保持动作可预测。尤其是”过近恢复”的短后退无法由前置摄像头确认机后净空；真机启用前必须确认后方、上方和下方搜索空间无遮挡，并随时准备按 `e` 急停。
